...
export default defineEventHandler(async (event) => {
	...
	if (body.newCategoryId) {
		...
		const newCategory = await Video.Category.findOne({
			where: {
		...
		await Video.Post.update({ categoryId: body.newCategoryId }, { where: { categoryId: params.id } });
	} else {
	...
	// Deleta a categoria
	const category = await Video.Category.destroy({
		where: {
			id: params.id,
		},
	});
