<template>
	<NuxtLink
		id="cardOne"
		:to="props.linkSource"
		class="w-full h-[calc(100vh-140px)] md:h-[calc(100vh-60px)] lg:h-auto relative grid grid-flow-col auto-cols-auto justify-between px-4 sm:px-6 md:px-8"
		:target="
			app.config_will_have_raffle && !app.config_will_have_carousel_banner_main
				? '_blank'
				: '_self'
		"
	>
		...
		<!-- gradiente -->
		<div
    class="absolute bottom-0 left-0 right-0 h-72 lg:h-40 bg-gradient-to-t from-black to-transparent"
		></div>

		<!-- Lado Esquerdo -->
		<div
			class="relative grid lg:grid-flow-col auto-cols-auto col-span-2 lg:items-center items-end"
		>
			<!-- Imagem de destaque -->
			<div
				v-if="props.hasImageDetach"
			...
			>
				<div class="text-start">
					...
					<h1
						...
						v-html="props.title"
						:style="textColorDetach"
					></h1>
					<!-- Subtítulo -->
					<p
						class="fm2 text-[10px] sm:text-[16px] md:text-lg lg:text-base leading-[.8rem] sm:leading-5 lg:leading-tight animate__animated animate__fadeInUp"
						v-html="props.subtitle"
					></p>

					<!-- Contagem Regressiva -->
					<div
						v-if="props.countdown"
						class="mt-1 text-[16px] sm:text-[28px] md:text-[34px] lg:text-[26px] font-bold"
					>
				...
				>
					...
					<UIcon
						v-if="props.callToAction"
						class="arrow ms-1 text-lg md:text-2xl"
			...
			<div v-if="!props.loading" class="space-y-2 opacity-30">
				<USkeleton
					class="h-2 sm:h-3 md:h-4 w-[80px] sm:w-[180px] md:w-[150px] xl:w-[200px] bg-skeleton"
					:ui="configSkeleton"
				/>
				<USkeleton
					class="h-2 sm:h-3 md:h-4 w-[70%] bg-skeleton"
					:ui="configSkeleton"
				/>
			</div>
		</div>
		...
		<div
			v-if="props.loading"
			class="w-[115px] sm:w-[220px] md:w-[240px] lg:w-[180px] col-span-1 flex items-end lg:items-center animate__animated animate__tada"
		>
			<img
				:src="props.imageAward"
...
const textColorDetach = computed(() => {
	if (
		...
		storeIncentive.nextDrawDateIsBefore(props.countdown)
	) {
